        ...
        <div class="content page-order-checkout checkout">
                    ...
                    <div class="box-inner js-checkout-address-panel">
                        <div class="address-common-table js-multiple-address-panel">
                            <ul class="address-item-list clear js-address-item-list">
                                <li class="js-choose-address " @click="chooseReceive(index)" :class="{'selected-address-item':receiveIndex===index}" v-for="(item,index) in receiveInfo" :key="(item,index)">
                                ...
                                <li class="add-address-item js-add-address" @click="openPop">
                                    <p>使用新地址</p>
                                </li>
                            </ul>
                        </div>
                    ...
                    <div class="invoice-detail">发票抬头：
                        <div class="radio-box">
                            <label>
                                ...
                                <span class="blue-radio" @click="checkedInvoice(true)" :class="{'blue-radio-on':incoice.personal}"><a></a></span>  个人
                            </label>
                            <label>
                                <input type="radio" class="hide">
                                <span class="blue-radio" @click="checkedInvoice(false)" :class="{'blue-radio-on':!incoice.personal}"><a></a></span>  单位
                            </label>
                        </div>
                        <div class="module-form-row form-item fn-hide js-invoice-title" v-if="!incoice.personal">
                            <div class="module-form-item-wrapper no-icon small-item">
                                <i v-show="!incoice.name">请填写公司发票抬头</i>
                                <input type="text" class="js-verify" v-model="incoice.name">
            ...
            <div class="gray-box">
                ...
                <div class="box-inner ui-goods-cart">
                    ...
                    <div class="cart-table" v-for="(item,index) in checkGoods" :key="(item,index)">
                ...
                <div class="box-inner">
                    <div class="last-payment clear">
                        <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrder"> <a>提交订单</a> </span>
                        <span class="prices fn-right">应付金额： <em>¥ {{checkedPrice + freight}}.00</em></span>
                    </div>
                </div>
            </div>
        </div>
        <addressPop v-if="showPop" @close="closePop"></addressPop>
...
export default {
    ...
    methods: {
        ...
        checkedInvoice (boole) {
          this.incoice.personal = boole
        },
        /* 提交订单 */
        submitOrder () {
          if (!this.incoice.personal && !this.incoice.name) {
            return
          }
          let receiverInfo = this.receiveInfo[this.receiveIndex]
          if (this.incoice.personal) {
            this.incoice.name = '个人'
          }
          let iData = new Date()
          let month = iData.getMonth() + 1
          let day = iData.getDate()
          if (month >= 1 && month <= 9) {
            month = '0' + month
          }
          if (day >= 1 && day <= 9) {
          ...
          let data = {
            orderId: iData.getTime(), /* getTime() 方法可返回距 1970 年 1 月 1 日之间的毫秒数 */
            goodsData: this.checkGoods,
            price: this.checkedPrice,
            freight: this.freight,
            invoiceName: this.incoice.name,
            iDate: iData.getFullYear() + '-' + month + '-' + day,
